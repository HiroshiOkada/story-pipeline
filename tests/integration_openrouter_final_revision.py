"""OpenRouter を使う全体改稿・完成判定の手動統合試験。"""
...
from story_pipeline.final_revision import DEFAULT_FINAL_REVISION_CONTEXT
...
def main() -> int:
    with tempfile.TemporaryDirectory() as directory:
        ...
        (root / "requests" / "0000.md").write_text(
            "# 作品作成要求\n\n採用済み全話を最終評価し、必要な箇所だけ局所改稿して作品を完成させてください。"
            "凪と湊が壊れた祭りの看板を直して友情を取り戻す一章完結の現代青春短編で、"
            "凪の三人称限定視点を維持し、超常現象と夢落ちは使わないでください。\n",
            encoding="utf-8",
        )
        documents = {
            ...
            "characters.md": "# 人物設定\n\n凪は慎重な高校生。湊は率直な幼なじみ。\n",
            "plot.md": "# 全体構成\n\n看板の共同修理を通じて口論の原因を認め、翌日の祭りで会う約束をする。\n",
            "style.md": "# 文体設定\n\n凪の三人称限定視点、過去時制。簡潔で感覚的に描く。\n",
            "canon.md": "# 確定事実\n\n開始時、二人は疎遠で、祭りの木製看板は壊れている。\n",
        }
        ...
        (root / "chapters" / "0001.md").write_text(
            ...
            "## 終了状態\n二人は和解。\n\n## 主要な出来事\n看板を共同で修理する。\n\n"
            "## 収録話\n0001-0002\n\n## 接続条件\n一章完結。\n\n"
            "## 完成後のあらすじ\n二人は看板を直しながら口論を謝り、翌日の祭りで会う約束をした。\n",
            encoding="utf-8",
        )
        (root / "episodes" / "0001.md").write_text(
            "## 話タイトル\n錆びた釘\n\n## 本文\n"
            "凪が倉庫へ入ると、湊は壊れた看板を抱えていた。二人は目を合わせず古い釘を抜いた。"
            "板を押さえる手が触れ、凪は去年の口論を謝った。湊も返事を待たず背を向けたことを認めた。\n",
            encoding="utf-8",
        )
        (root / "episodes" / "0002.md").write_text(
            "## 話タイトル\n新しい文字\n\n## 本文\n"
            "凪と湊は看板へ祭りの名を塗り直した。夕日で文字が乾くころ、湊が明日も一緒に来ようと言った。"
            "凪はうなずき、二人で修理した看板を入口へ掲げた。潮風の中、新しい文字がまっすぐ光った。\n",
            encoding="utf-8",
        )
        request = select_request(root, load_state(root))
        interpretation = parse_request_interpretation(json.dumps({
            "kind": "continue", "summary": "短編全体を最終評価して完成させる", "targets": [],
            "required_conditions": ["友情の回復", "共同修理", "一章完結", "三人称限定視点"],
        ...
        config = {
            ...
            "models": {"final-revision-integration": {
                "provider": "openrouter", "model": "deepseek/deepseek-v4-flash:nitro",
                "max_tokens": 4096, "parameters": {"temperature": 0},
            }},
            "roles": {role: ["final-revision-integration"] for role in ("reviewer", "reviser")},
            "limits": {
                "generation_calls": 3, "review_calls": 4, "revision_calls": 3,
                "retry_calls_per_request": 2,
            },
            "request": {"timeout_seconds": 120, "retry_attempts": 2},
        }
        result = produce_final_revision(
            root, request, interpretation, LLMClient(config, load_environment(config)),
        ...
        if result.status != "completed" or result.best is None or result.completion_update is None:
            raise RuntimeError(f"final revision did not complete: {result.status} {result.reason}")
        if result.completion_update.phase != "completed":
            raise RuntimeError("final revision returned an unexpected phase")
        print(
            "OpenRouter final revision passed: "
