...
`failed` または `awaiting_human` でも、報告と再開情報を終了時コミットして次要求を作成する。`failed` を同じ要求から再開する間、次要求は空のテンプレートとして残す。人間が先に次要求を書き始めた場合は順序を曖昧にしないため再開せず、内容を退避するよう案内する。`awaiting_human` では次要求を回答要求として処理する。
