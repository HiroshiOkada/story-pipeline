...
5. `completed` の場合だけ次の要求テンプレートを未コミットで作成する。
6. ロックを解放する。

`failed` または `awaiting_human` では、報告と再開情報を終了時コミットするが、次要求の作成は人間の回答が必要な `awaiting_human` の場合に限る。API 障害など同じ要求を再開する `failed` では作成しない。
