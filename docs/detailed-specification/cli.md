...
成功時は要求番号、status、変更ファイル、使用した各 role のモデル、論理呼び出しと transport 試行数、再試行待機と所要時間、報告パス、次要求パスを表示する。失敗時は秘密を除いた原因、報告パス、再開方法を標準エラーへ表示する。予期しない障害では traceback を表示せず、run JSON と共通の incident ID と component を表示する。
